...
def test(model, device, val_dataloader, epoch):
    ...
    with torch.no_grad():
        ...
        print('Validation Accuracy of the model on the {} validation images: {} %'.format(total, 100 * accuracy))
...
if __name__ == "__main__":
    ...
    N_data = len(dataset)
    train_dataset, val_dataset = utils.data.random_split(dataset, [int(N_data * 0.8), len(dataset) - int(N_data * 0.8)])
