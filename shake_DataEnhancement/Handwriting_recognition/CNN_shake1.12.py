...
def print_random_rotation_info(rotation_collator):
    print("\n[RandomRotation]")
    print(f"Degrees range: {rotation_collator.degrees}")
    print(f"Max rotation angle: {max(rotation_collator.degrees)}")


def print_random_horizontal_flip_info(flip_collator):
    print("\n[RandomHorizontalFlip]")
    print(f"Flip probability p: {flip_collator.p}")
...
if __name__ == "__main__":
    ...
    rotation_collate_fn = RandomRotationBatchCollator(degrees=(0, 5))
    ...
    flip_collate_fn = RandomHorizontalFlipBatchCollator(p=0.1)
    ...
    model_random_rotation = ConvNet().to(device)
    model_random_horizontal_flip = ConvNet().to(device)

    criterion = nn.CrossEntropyLoss()

    optimizer_plain = optim.Adam(model_plain.parameters(), lr=1e-4)
    optimizer_aug = optim.Adam(model_aug.parameters(), lr=1e-4)
    optimizer_random_rotation = optim.Adam(model_random_rotation.parameters(), lr=1e-4)
    optimizer_random_horizontal_flip = optim.Adam(model_random_horizontal_flip.parameters(), lr=1e-4)
    ...
    print("Training RandomRotation model...")
    for epoch in range(num_epochs):
        model_random_rotation.train()
        for x, y in train_loader_random_rotation:
            x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True)
            optimizer_random_rotation.zero_grad()
            loss = criterion(model_random_rotation(x), y)
            loss.backward()
            optimizer_random_rotation.step()
        print(f"Epoch {epoch+1} | RandomRotation Acc: {accuracy(model_random_rotation, device, test_loader)*100:.2f}%")

    print("Training RandomHorizontalFlip model...")
    for epoch in range(num_epochs):
        model_random_horizontal_flip.train()
        for x, y in train_loader_random_horizontal_flip:
            x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True)
            optimizer_random_horizontal_flip.zero_grad()
            loss = criterion(model_random_horizontal_flip(x), y)
            loss.backward()
            optimizer_random_horizontal_flip.step()
        print(f"Epoch {epoch+1} | RandomHorizontalFlip Acc: {accuracy(model_random_horizontal_flip, device, test_loader)*100:.2f}%")
    ...
    print_random_rotation_info(rotation_collate_fn)
    print_random_horizontal_flip_info(flip_collate_fn)
    ...
    torch.save(model_random_rotation.state_dict(), "checkpoint/random_rotation.pth")
    torch.save(model_random_horizontal_flip.state_dict(), "checkpoint/random_horizontal_flip.pth")
