...
if __name__ == "__main__":
    ...
    train_aug = ShakeAugDataset(
        base=train_base,
        recipes=recipes,
        include_original=True,     # batch 中既有原图也有增强图
        dynamic=True,              # 每个 epoch 变化
        base_seed=0,
        apply_in_getitem=False,    # ❗关键：不在 __getitem__ 里增强
    )
    ...
    train_loader_aug = DataLoader(
        ...
        num_workers=4,
        pin_memory=True,
        collate_fn=collate_fn,     # ❗关键：batch 级增强
    )
    ...
    for epoch in range(num_epochs):
        train_aug.set_epoch(epoch)   # ❗关键：驱动 dynamic=True
        model_aug.train()
