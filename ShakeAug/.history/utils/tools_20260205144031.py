...
class EarlyStopping:
    """
    Early stopping by accuracy (higher is better).

    Saves:
      - checkpoint_best.pth (always overwrite, easiest to load)
      - checkpoint_best_acc_{acc:.4f}.pth (ONLY keep latest best; delete old one)
    """
    ...
    def __call__(self, acc, model, path):
        acc = float(acc)
        score = acc  # higher is better

        if self.best_score is None:
            self.best_score = score
            self._save_checkpoint(acc, model, path)
            return
    ...
    def _save_checkpoint(self, acc, model, path):
        os.makedirs(path, exist_ok=True)

        if self.verbose:
            print(f"Accuracy increased ({self.best_acc:.6f} --> {acc:.6f}). Saving model ...")

        # stable best path
        best_path = os.path.join(path, "checkpoint_best.pth")
        torch.save(model.state_dict(), best_path)

        # delete old acc file
        if self._last_acc_ckpt_path is not None and os.path.exists(self._last_acc_ckpt_path):
            try:
                os.remove(self._last_acc_ckpt_path)
            except OSError:
                pass

        # save new acc file (only keep latest)
        acc_path = os.path.join(path, f"checkpoint_best_acc_{acc:.4f}.pth")
        torch.save(model.state_dict(), acc_path)
        self._last_acc_ckpt_path = acc_path
