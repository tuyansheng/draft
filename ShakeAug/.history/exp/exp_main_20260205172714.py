...
class Experiment_Main(Experiment_Basic):
    ...
    # aug_mode: none / traditional / shake
    def _build_transforms(self, dataset_name: str, is_train: bool):
        ...
        if dataset_name in ["mnist", "minist"]:
            if (not is_train) or (aug_mode in ["none", "shake"]):
                return baseline_mnist()

            # traditional aug (train only) - MNIST fixed 28
            return transforms.Compose([
        ...
        if dataset_name in ["cifar10", "cifar100"]:
            ...
            # traditional aug (train only) - CIFAR fixed 32
            return transforms.Compose([
        ...
        if dataset_name == "imagenet":
            ...
            if aug_mode in ["none", "shake"]:
                # train baseline deterministic (no RandomResizedCrop)
                return transforms.Compose([
                    transforms.Resize(256),
                    transforms.CenterCrop(img_size),
                    transforms.ToTensor(),
    ...
    def _get_data(self, flag):
        ...
        # Traditional: explicit data expansion by repeating dataset
        # trad_repeat=1 -> no expansion (N)
        # trad_repeat=R -> expanded to R*N (RepeatAug)
        if is_train and aug_mode == "traditional":
        ...
        if is_train and aug_mode == "shake":
            ...
            # include_original=True => dataset becomes (K+1)x
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            #     save_path = os.path.join(path, f"{metrics_test['accuracy']:.4f}_{timestamp}.pth")
